...
export const imageService = {
  ...
  getDestinationImage(destination?: Destination): string {
    ...
    return destinationImages.find(isUsableImageUrl)?.trim() ?? DEFAULT_TRAVEL_IMAGE_URL;
  },

  getDiaryEntryImage(destination: Destination, stopIndex: number): string {
    const imageUrls = Array.isArray(destination.imageUrls) ? destination.imageUrls.filter(isUsableImageUrl) : [];
  ...
  normalizeDiaryEntries(entries: DiaryEntry[], destination: Destination): DiaryEntry[] {
    return entries.map((entry, index) => ({
      ...entry,
      imageUrl: isUsableImageUrl(entry.imageUrl) ? entry.imageUrl.trim() : this.getDiaryEntryImage(destination, index + 1),
    }));
  },
